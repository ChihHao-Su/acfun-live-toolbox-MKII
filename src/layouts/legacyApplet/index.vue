<template>
	<div id="home">
		<div class="minify-bar">
			<handler-moving class="handler">{{ $route.query.name }}</handler-moving>
			<div class="button el-icon-refresh-right" @click="reset()" title="刷新" />
			<div class="button el-icon-setting" @click="openSetting" title="设置" />
			<div class="button el-icon-monitor" @click="openConsole" title="控制台" />
			<div
...
import { setTop, minimize, close, openConsole, readConfig, saveLegacyApplet as saveLegacyAppletSettings } from "@front/util_function/system";
...
export default defineComponent({
	...
	mounted() {
		...
		registerRole(name);
		wsevent.register(this.appID, this.$store.state.danmakuProfile?.general?.socket || 4396);
		path = decodeURIComponent(path);
		loadComponent(path).then((res) => {
			const config = JSON.parse(configurations);
			let obsLink = "";
			if (config.obsPath) {
				obsLink = `${window.location.origin}/obs/legacyApplets/?name=${name}&path=${encodeURIComponent(config.obsPath)}`;
			}
			...
			const methods = {
				...
				system,
				loadSettings: () => {
					return loadLegacyAppletSettings(name);
				},
				saveSettings: (data) => {
					return saveLegacyAppletSettings(name, data);
				},
			};
